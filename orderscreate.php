...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    do {
        if (empty($customerID) || empty($carID) || empty($title) || empty($description) || empty($status)) {
            $errorMessage = "All the fields are required (Missing Field) Status: $status";
            break;
        }

        // Add new client to database
        $sql = "INSERT INTO order_table (orderID, customerID, carID, title, description, status, approved, finished, dateBegun, parts)" .
            "VALUES ('$orderID', '$customerID', '$carID', '$title', '$description', $status, $approved, '$finished', '$dateBegun', '$parts')";
